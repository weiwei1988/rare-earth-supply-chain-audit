...
export function integrate(source, original, subcategories) {
  ...
  for (const row of source.rows) {
    ...
    const provenance = { corporateNumber: String(row[2] ?? ''), sourceName: row[1], row: rowNumber,
      sourceUrl: `https://docs.google.com/spreadsheets/d/${source.spreadsheetId}/edit#gid=${source.sheetId}&range=A${rowNumber}:AH${rowNumber}`,
      decision: row[26], userReason: row[27] ?? '', years: row[4], readAt: source.readAt };
    if (!selected) {
    ...
    Object.assign(company, {
      ...
      src: [company.src, provenance.sourceUrl, row[25], row[33]].filter(Boolean).join(' ／ '),
      note: [String(company.note || '').split(' ／ ').filter(s => !/^売上根拠：|^希土類フラグはStage/.test(s)).join(' ／ '), `人力確認済み工程5判定：対象。${row[27] || ''}`, `希土類判定：${row[30]}。${row[31] || ''}`, report.specialCases.some(s => s.row === rowNumber) ? '蓄電池をユーザー指定でSOFC／SOEC分類へ暫定収容。SOFC製品ではない。' : ''].filter(Boolean).join(' ／ '),
    });
