...
test('公開データへ元Spreadsheetの識別子・行番号・自由記述を持ち込まない', () => {
  const r = row(1, '株式会社検証甲', '対象', '05_robot ロボティクス（該当品目：ロボット）', 'Sm');
  r[27] = '公開してはいけない内部判定メモ';
  const result = integrate(source([r]), [], subs);
  const company = result.companies[0];
  assert.equal(company.atla.row, undefined);
  assert.equal(company.atla.sourceUrl, undefined);
  assert.equal(company.atla.userReason, undefined);
  assert.equal(JSON.stringify(company).includes('docs.google.com/spreadsheets/'), false);
  assert.equal(JSON.stringify(company).includes('公開してはいけない内部判定メモ'), false);
});
