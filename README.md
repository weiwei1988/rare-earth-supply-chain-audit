...
- 初期表示は100%です。カード幅と列間隔は画面幅に合わせて配分し、標準画面では6工程を横方向に収めます。詳細は拡大操作で確認できます。
- 「全体」ではすべての接続線を表示し、サブカテゴリーを選ぶと、同じ元素でつながる全上流・全下流工程の線を強調します。
- 「防衛装備庁調達実績あり」は、人力更新シートに調達実績がある企業の表示です。詳細欄で調達品目判定と希土類判定を確認できます。
...
画面のレイアウトや描画ロジックは、`index.html` とJSXの生成マーカー外を編集します。サブカテゴリーを追加・変更する場合は、対象元素と上流の `src` 接続、各 `src` に対応する `srcEls` の希土類、`srcNotes` の接続根拠を更新してください。工程を飛ばす接続には `srcSkip` で理由を記録します。用途と関係のない共通元素を自動接続しないでください。未確認のBOM、調達国、防衛契約、企業関係を推測で確定しないでください。
